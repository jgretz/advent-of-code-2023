...
function isPartValid(world: World, part: Part): boolean {
  return world.signs.some((sign) => doesSignTouchPart(sign, part));
}

function findAllValidParts(world: World): Part[] {
  return world.parts.filter((part) => {
    const isValid = isPartValid(world, part);

    return isValid;
  });
}

function sumParts(parts: Part[]): number {
  return parts.reduce((sum, part) => sum + part.id, 0);
}

function main() {
  of(testData)
    .pipe(map(parse))
    .pipe(map(findAllValidParts))
    .pipe(map(sumParts))
